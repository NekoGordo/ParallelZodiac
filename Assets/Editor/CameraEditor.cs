...
public class CameraEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        if (!EditorGlobals.HideEditors)
        {
            ...
            {
                EditorGlobals.ShowConstraints("Camera-Player Distance", ref self.MIN_CAM_DIST, ref self.MAX_CAM_DIST);
            }
    ...
    {
        float x = self.sphericalPos.x * Mathf.Sin(self.sphericalPos.y * Mathf.Deg2Rad) * Mathf.Sin(self.sphericalPos.z * Mathf.Deg2Rad);
        float y = self.sphericalPos.x * Mathf.Cos(self.sphericalPos.y * Mathf.Deg2Rad);
        float z = self.sphericalPos.x * Mathf.Sin(self.sphericalPos.y * Mathf.Deg2Rad) * Mathf.Cos(self.sphericalPos.z * Mathf.Deg2Rad);
    ...
    Vector3 ShowSphericalPoint(string label, string R, string T, string P, Vector3 point)
    {
        ...
        if (dynamicPoint.x < self.MIN_CAM_DIST) dynamicPoint.x = self.MIN_CAM_DIST; else if (dynamicPoint.x > self.MAX_CAM_DIST) dynamicPoint.x = self.MAX_CAM_DIST;
